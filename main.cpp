...
class ZBuffSearcher {
  ...
  public:
    ZBuffSearcher(const uintmax_t _buffSz, std::vector<ATZdata::streamOffset> *p) : ZOBuffSz{_buffSz},
...
#ifdef debug
                std::cout << std::endl;
                std::cout << "stream #" << i << "(" << streamOffsetList[i].offset << ")"
                          << " ready for recompression trials" << std::endl;
#endif // debug
...
#ifdef debug //if the most probable parameters are not succesful, try all different clevel and memlevel combinations
        std::cout << "   trying less probable parameters: fast compression" << std::endl
                  << std::endl;
#endif // debug
...
#ifdef debug
        std::cout << "   trying less probable parameters: default compression" << std::endl
                  << std::endl;
#endif // debug
        if (testDeflateParams(rBuffer, decompBuffer, streamobj, zlibparams2)) return;
...
//if the most probable parameters are not succesful, try all different clevel and memlevel combinations
#ifdef debug
        std::cout << "   trying less probable parameters: best compression" << std::endl
                  << std::endl;
#endif // debug
...
#ifdef debug
    std::cout << std::endl
              << "Original file size:" << infileSize << std::endl;
    std::cout << std::endl
              << "Reconstructed file size:" << recfileSize << std::endl;
#endif // debug
