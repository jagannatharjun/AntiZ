...
public:
  ...
  // after Operation if avail_out is zero, call continuePrev to continue Operation
  // return inflate() return value
  auto operator()(void *dest, size_type destlen, void *src, size_type srclen,decltype(Z_SYNC_FLUSH) FlushType = Z_SYNC_FLUSH) {
    LastRet = inflateReset(&strm);
    if (LastRet != Z_OK) throw errToString(LastRet);
  ...
  auto continuePrev(void *dest, size_type destlen,decltype(Z_SYNC_FLUSH) FlushType = Z_SYNC_FLUSH) {
    strm.next_out = reinterpret_cast<byteP>(dest);
    strm.avail_out = LastDestLen = destlen;
    return LastRet = inflate(&strm, FlushType);
  }

  auto refillInput(void *src,size_type srcLen,decltype(Z_SYNC_FLUSH) FlushType = Z_SYNC_FLUSH) {
